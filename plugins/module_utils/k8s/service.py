...
class K8sService:
    ...
    def create_project_request(self, definition: Dict) -> Dict:
        ...
        if not self.module.check_mode:
            try:
                results["result"], results["warnings"] = decode_response(
                    self.client.create(resource, definition, serialize=False)
                )
    ...
    def patch_resource(
        ...
        try:
            ...
            return self.decode_response(
                self.client.patch(resource, definition, **params)
            )
        except Exception as e:
            reason = e.body if hasattr(e, "body") else e
...
def decode_response(resp) -> Tuple[Dict, List[str]]:
    k8s_obj = ResourceInstance(None, json.loads(resp.data.decode("utf8"))).to_dict()
    warnings = resp.headers["warning"].split(", ") if "warning" in resp.headers else []
    return k8s_obj, decode_warnings(warnings)
...
def decode_warnings(warnings: str) -> List[str]:
    ...
    for warning in warnings:
        ...
        if m:
            try:
                parsed, _ = parse_quoted_string(m.group(1))
    ...
    for b in quoted_string:
        if b == '"':
            if escaping:
                ...
                closed_quote = True
                remainder = quoted_string[quoted_string.index(b) + 1 :].strip()
                break
        elif b == "\\":
            if escaping:
                result.append(b)
